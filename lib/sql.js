...
function initDatabase(database, config) {
    ...
    database.serialize(function () {
        database.exec(sql, function (err) {
            if (err) {
                console.log("database.exec(sql", err);
            }
        });
    });
}
...
function insertCard(database, deck, card) {
    var createTime = card.id || +new Date();
    ...
    database.serialize(function () {
        ...
        database.run(SQL_CARD, [cardId, noteId, deck.id, createTime], function (err) {
            if (err) {
        ...
        database
            ...
            fieldsContent,
            sortField,
            parseInt(sha1(fieldsContent).substr(0, 8), 16)], function (err) {
            if (err) {
